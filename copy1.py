...
folder_list.sort()
for i in range(502, 1571, 1):
    path = os.path.join(workdir, str(folder_list[i]), "images")
    ...
    for m in range(len(file_list)):
        if file_list[m].endswith(".png"):
            ...
            filename2 = os.path.join(workdir1, "cam{0:02d}".format(m+1), "frame_{0:05d}.jpg".format(i - 502 + 1))
